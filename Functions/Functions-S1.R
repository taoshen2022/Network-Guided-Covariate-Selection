...
NGCS <- function(A, X, K, Lap = TRUE, Standardize = TRUE, S = NULL, alpha = 1/2, sigma = 1, pvalcut = NULL, Type = "eigen"){
  ...
  if (Type == "eigen"){
    ...
    if (K <=5){
      pval_han = pmin(exp(-4*pmin((t/2-K)^2/(K^2*sigma^4), abs(t/2-K)/sigma^2)), 1);
    }else{
      pval_han = pmin(exp(-4*pmin((t-K)^2/(K^2*sigma^4), abs(t-K)/sigma^2)), 1);
    }
    if(!is.null(S)){
      psort = sort(pval, decreasing = FALSE);
      selected = which(pval <= psort[S]) #It will keep all the ties
      return(list(pval = pval, selected = selected, hc = NULL, S = S, K = K, sta = t, hpval = pval_han))
    }
  }
  if (Type == "lsingular"){
  ...
  #the selection is based on chisq in general, for HW-chi-p values, we report p-values and the selction should be further made by applying HC function.
  HCresult = HCfs(pval, alpha = alpha, pvalcut = pvalcut);
  HC = HCresult$HC;
  S = HCresult$S; 
  selected = HCresult$selected
  return(list(pval = pval, selected = selected, hc = HC, S = S, K = K, sta = t, hpval = pval_han))
...
HCfs <- function(pval, alpha = 1/2, pvalcut = NULL){
  ...
  n = length(pval);
  if(is.null(pvalcut)){pvalcut = 1/n;}
...
rE_isotropic_mix <- function(n, p, sigma = 1,
                            types = NULL,
                            pool = c("rademacher","uniform","bern01","spike"),
                            prob = NULL,
                            q_spike = 0.02,    # P(+/-c) each for "spike"; mass at 0 is 1-2q
                            p_bern = 0.5,      # P(1) for "bern01"
                            center_rows = TRUE,
                            exact_isotropy = FALSE) {
  if (is.null(types)) types <- sample(pool, p, TRUE, prob)
  gen_col <- function(t) {
    t <- tolower(t)
    if (t == "rademacher") {
      # ±1 scaled to sigma
      sigma * sample(c(-1, 1), n, TRUE)
    } else if (t == "uniform") {
      # Unif[-a,a], Var=a^2/3=sigma^2
      a <- sqrt(3) * sigma
      runif(n, -a, a)
    } else if (t %in% c("bern01","bernoulli01")) {
      # 0/1 centered & scaled to Var = sigma^2
      b <- rbinom(n, 1, p_bern)
      sigma * (b - p_bern) / sqrt(p_bern * (1 - p_bern))
    } else if (t == "spike") {
      # Three-point: {+c, 0, -c} with probs (q, 1-2q, q); mean 0, Var = 2 q c^2 = sigma^2
      cval <- sigma / sqrt(2 * q_spike)
      z <- sample(c(-cval, 0, cval), n, TRUE, c(q_spike, 1 - 2*q_spike, q_spike))
      z
    } else stop("Unknown type: ", t)
  }
  E <- matrix(NA_real_, n, p)
  for (j in seq_len(p)) E[, j] <- gen_col(types[j])
  
  if (center_rows) E <- scale(E, center = TRUE, scale = FALSE)
  
  if (exact_isotropy) {
    S <- cov(E)
    ev <- eigen(S, symmetric = TRUE)
    E <- E %*% ev$vectors %*% diag(1 / sqrt(pmax(ev$values, 1e-12))) %*% t(ev$vectors) * sigma
  }
  list(E = E, types = types)
}
